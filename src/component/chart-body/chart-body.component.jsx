...
const ChartBodyComponent = () =>{
  ...
  const chartSeries = useRef(null);
  useEffect(()=>{
    if(chartRef.current && chartDimension.height!==0 && count === 0){
      chartTemp.current = createChart(chartRef.current,{
  ...
  const handleSize = () =>{
    chartTemp.current.resize(chartContainerRef.current?.clientWidth,chartContainerRef.current?.clientHeight)
  }
  useEffect(()=>{
    setChartDimension({width: chartContainerRef.current?.clientWidth, height: chartContainerRef.current?.clientHeight})
    count === 0?console.log():alert()
  },[chartContainerRef.current])


  useEffect(()=>{
    !chartTemp.current?console.log():selectChart(chartTemp.current);
  },[charts.displayChart]);
  const selectChart = (chart) =>{
    switch (charts.displayChart) {
      case "Candle":
        return prepareChart(chart);
      case "Bar":
        return prepareBarChart(chart);
      case "Mountain":
        return prepareAreaChart(chart);
      default:
        return;
    }
  };
  function prepareBarChart(chart){
    chartSeries.current === null?console.log():chart?.removeSeries(chartSeries.current);
    chartSeries.current = chart.addBarSeries();
    let data = [...dataM];
    chartSeries.current.setData(data);
  }
  function prepareAreaChart(chart){
    chartSeries.current === null?console.log():chart.removeSeries(chartSeries.current);
  ...
  function prepareChart(chart) {
    chartSeries.current === null?console.log():chart.removeSeries(chartSeries.current);
    chartSeries.current = chart.addCandlestickSeries();
    let data = [...dataM];
    chartSeries.current.setData(data);
  ...
  return(
    ...
    >
      {/*<ResizeDetector handleHeight handleWidth onResize={dimensionHandler}/>*/}
      <div ref={chartRef}/>
    </div>
  )
}
...
export default ChartBodyComponent
